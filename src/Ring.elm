...
fromList list =
  let
    nonemptyMembers = case NEA.fromList list of
      Just a -> a
      Nothing -> Debug.crash "Don't initialize a ring with an empty list"
  in
...
advance ring =
  { ring | currentIndex = nextIndex ring
...
nextIndex ring =
  if
    ring.currentIndex == (NEA.length ring.members - 1)
  then 0
  else ring.currentIndex + 1
